 ...
 *     Caching multi-MB binary bodies (event photos) or notification streams
 *     is at best wasted, and at worst stalls responses because
 *     `response.clone()` tees the underlying network stream until the
 *     cache.put drains.
 */

const SW_URL = new URL(self.location.href);
const SW_VERSION = SW_URL.searchParams.get('v') || 'dev';
const CACHE_NAME = `familycircle-static-${SW_VERSION}`;
const RUNTIME_CACHE = `familycircle-runtime-${SW_VERSION}`;
...
self.addEventListener('fetch', (event) => {
  ...
  // skip caching event photo bytes and any future streaming endpoints —
